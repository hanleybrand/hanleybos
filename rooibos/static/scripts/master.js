        ...
        if (mdid.debug === true) console.log('mdid.debug: ' + mdid.debug);


        function basket_imgs_first_row_top() {
            if (mdid.debug === true) console.log('basket_imgs_first_row_top');
            return ($("#basket-thumbs img:visible:first").position() || Object({top: 0})).top;
        }

        function basket_scroll_arrows() {
            if (mdid.debug === true) console.log('basket_scroll_arrows');
        ...
        function recordSelection(ids, checked) {
            if (mdid.debug === true) console.log('recordSelection' + ids, checked);
...
//              url: '{% url ui-api-select-record %}',
        ...
        function bindSelectRecordCheckboxes() {
            if (mdid.debug === true) console.log('bindSelectRecordCheckboxes');
            $(".record-select").click(function() { recordSelection($(this).attr('value'), $(this).attr('checked')); });
        }


        function add_metadata_popup(context) {
            if (mdid.debug === true) console.log('add_metadata_popup ' + context);
            $(".metadata-anchor[id*=record-id-]", context).not(".no-metadata-popup").each(function() {
                ...
                anchor.qtip({
                    content: {
                        url: mdid.dataRecordPreview.replace('/0/', '/' + id + '/')
                    },
                    show: {
                        delay: 1000,
                        solo: true
        ...
        function store_profile_setting(key, value) {
            if (mdid.debug === true) console.log('store_profile_setting ' + key, value );
            ajaxInteractionManager.add({
                type: 'POST',
                url: mdid.URLprofileStore,
        ...
        function process_facets() {
            if (mdid.debug === true) console.log('process_facets');
            var freqs = Object();
        ...
        function addCopyLinkIcon(item) {
            if (mdid.debug === true) console.log('addCopyLinkIcon');
            ...
            var link = $("<img class='item-action-link' src='" + mdid.staticUrl + "' />");
            link.qtip({
        ...
        function keepAtTop(element, not_past) {
            if (mdid.debug === true) console.log('keepAtTop(element, not_past): ' + element, not_past);
        ...
        $(document).ready(function() {
            if (mdid.debug === true) console.log('master.html doc ready');
            var timeouts = Object();
            ...
            function hideBasket(immediate) {
                if (mdid.debug === true) console.log('hidebasket');
            ...
            function showBasket() {
                if (mdid.debug === true) console.log('showBasket');
            ...
            $("#basket-toggle").click(function(e) {
                if (mdid.debug === true) console.log('basket-toggle clicked');
                e.preventDefault();
            ...
            $("#basket-content").droppable({
                ...
                activate: function(event, ui) {
                    if (mdid.debug === true) console.log('activate (#basket-content).droppable');
            ...
            $("table.itemactions tr+tr").each(function() {
                var r = $(this);
                var c = $("<td class='item-actions-menu'><img src='" + mdid.staticUrl +  "' /><" + "/td>");
                var u = $("<ul><" + "/ul>");
        ...
        // pop-up preview
         ...
         var thumb_url = "{% url storage-thumbnail 0, 0 %}";

         function preview_dialog(a, img, prev, next, position, total) {
             var dialog = $("#preview-dialog");
             dialog.dialog("destroy").empty();
             var img_src = img.attr('src').substring(thumb_url.indexOf('/0/0/'));
             var qs_index = img_src.indexOf('?');
             if (qs_index > 0) img_src = img_src.substring(0, qs_index);
             var url = media_url.replace(/\/0\/0\//, img_src);
             ...
             //todo this needs to be fixed for django but for now hardcode
             // was:          var info = $('<img src="{% url static 'images/info.png' %}" alt="Information" class="info-icon metadata-anchor" />');

             var info = $('<img src="' + mdid.staticUrl + '" alt="Information" class="info-icon metadata-anchor" />');
